...
const Logo: React.FC<LogoProps> = ({ className = "", showText = false }) => {
  return (
    <div className={`flex items-center gap-4 cursor-default select-none group ${className}`}>
      ...
      {/* Text Part: "Show" with gradient */}
      {showText && (
        <span className="text-[40px] font-black tracking-tight leading-none text-transparent bg-clip-text bg-gradient-to-r from-[#3b27b2] via-[#8227b2] to-[#b61cc9]">
          Show
        </span>
      )}
    </div>
