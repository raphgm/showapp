...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete, onCancel }) => {
  ...
  const handleNext = async () => {
    // On the identity step (step 0), create the Firebase account
    if (step === 0) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setEmail(u.email || '');
      setStep(1); // skip to workspace step
    } catch (err: any) {
            ...
            {step === 0 && (
              <div className="space-y-6">
                ...
                <button
                  ...
                  type="button"
                  className="w-full flex items-center justify-center gap-3 py-4 border-2 border-slate-50 rounded-2xl hover:border-indigo-100 hover:bg-indigo-50/50 transition-all disabled:opacity-50"
                >
                  ...
                  <span className="text-sm font-black text-slate-600">Sign up with Google</span>
                </button>
              </div>
            )}
