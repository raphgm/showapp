  ...
  const groups = [
    {
      ...
      items: [
        ...
        { id: 'new-space', label: 'Create new space', icon: Plus, action: () => onNavigate(AppRoute.SPACES_ALL), shortcut: 'C' },
        { id: 'invite', label: 'Invite team members', icon: Users, action: () => onNavigate(AppRoute.SETTINGS), shortcut: 'I' },
      ]
    },
    {
      label: "Navigation",
    ...
    },
    {
      ...
      items: videos.slice(0, 3).map(v => ({
        ...
        label: v.title,
        icon: Video,
        action: () => onNavigate(AppRoute.LIBRARY),
        meta: v.duration
      }))
    },
    {
      ...
      items: [
        ...
        { id: 'billing', label: 'Billing & Plans', icon: CreditCard, action: () => onNavigate(AppRoute.BUY_OPTIONS) },
        { id: 'theme', label: 'Toggle Dark Mode', icon: Moon, action: () => { document.documentElement.classList.toggle('dark'); } },
      ]
    }
  ];
