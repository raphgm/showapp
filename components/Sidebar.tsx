...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const NavButton: React.FC<{ item: any }> = ({ item }) => (
    ...
    >
      {item.icon && <item.icon className={`size-4.5 ${currentRoute === item.id ? 'text-white' : 'text-slate-300 group-hover:text-indigo-400'}`} />}
      ...
      <aside className="w-[84px] border-r border-slate-100 bg-white h-screen flex flex-col sticky top-0 shrink-0 z-20 items-center py-6 overflow-y-auto no-scrollbar">
        <div className="flex-1 w-full space-y-3 flex flex-col items-center px-3">
          ...
          <button 
            ...
            className="size-11 rounded-2xl flex items-center justify-center text-white bg-gradient-to-br from-[#3b27b2] via-[#8227b2] to-[#b61cc9] font-black shadow-lg shadow-indigo-100 mb-6">
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-8 pb-6 no-scrollbar">
        ...
        {(user.role === 'Owner' || user.role === 'Admin') && (
          <div className="pt-2">
            ...
            >
              <Shield className={`size-4.5 ${currentRoute.startsWith('admin') ? 'text-white' : 'text-slate-500'}`} />
