...
const Library: React.FC<LibraryProps> = ({ videos, snippets = [], onVideoClick, onDeleteVideo, workspaceName, onBuyOptions, variant = 'default', coverImage }) => {
    ...
    return (
      <div className="max-w-6xl mx-auto py-10 px-8 space-y-12 animate-in fade-in duration-700">
        <HeroBanner
          ...
          buttons={
            <button
              onClick={() => { if (videos.length > 0) onVideoClick(videos[0]); }}
              className="flex items-center gap-3 bg-white text-indigo-900 px-8 py-4 rounded-2xl text-sm font-black uppercase tracking-wider hover:bg-indigo-50 transition-all shadow-xl active:scale-95"
            >
              ...
              videos.map((v, i) => (
                <div key={v.id} className="group bg-white border border-slate-100 p-8 rounded-[48px] hover:shadow-2xl hover:border-indigo-100 transition-all flex flex-col md:flex-row gap-10">
                  ...
                  <div className="flex flex-col justify-center gap-3 shrink-0">
                    ...
                    <button
                      onClick={() => {
                        const text = v.aiSummary || v.description || v.title;
                        navigator.clipboard.writeText(text);
                      }}
                      className="px-8 py-4 bg-white border border-slate-100 text-slate-400 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] hover:border-indigo-200 hover:text-indigo-600 transition-all"
                    >
                      Copy Recap
                    </button>
                  </div>
  ...
  return (
    <div className="max-w-7xl mx-auto py-10 px-8 space-y-12 animate-in fade-in duration-700 relative min-h-full">
      ...
      <HeroBanner
        ...
        imageUrl={coverImage || "https://images.unsplash.com/photo-1506744038136-46273834b3fb?auto=format&fit=crop&q=80&w=1200"}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 relative z-10">
        {[
          { label: 'STORAGE_UTILIZED', val: videos.length > 0 ? `${(videos.length * 0.8).toFixed(1)} GB` : '0 GB', icon: Database, color: 'text-indigo-600', detail: videos.length > 0 ? `${Math.min(Math.round((videos.length * 0.8 / 50) * 100), 100)}% CAPACITY` : 'EMPTY' },
          { label: 'PLAYLIST_SETS', val: String(collections.length).padStart(2, '0'), icon: Presentation, color: 'text-amber-500', detail: collections.length > 0 ? 'SYNCED' : 'NO SETS' },
          { label: 'SHOW_ASSETS', val: String(videos.length).padStart(2, '0'), icon: VideoIcon, color: 'text-emerald-500', detail: videos.length > 0 ? 'INDEXED' : 'NO ASSETS' }
        ].map((stat, i) => (
          <div key={i} className="bg-white border border-slate-100 rounded-[48px] p-10 shadow-sm flex flex-col justify-between group hover:shadow-2xl hover:border-indigo-100 transition-all relative overflow-hidden">
            ...
            <div className="space-y-3">
              ...
              <div className="h-1.5 w-full bg-slate-50 rounded-full overflow-hidden">
                <div className={`h-full ${i === 0 ? 'bg-indigo-600' : i === 1 ? 'bg-amber-400' : 'bg-emerald-500'} rounded-full transition-all duration-1000`} style={{ width: i === 0 ? `${Math.min(Math.round((videos.length * 0.8 / 50) * 100), 100)}%` : i === 1 ? `${collections.length > 0 ? Math.min(collections.length * 20, 100) : 0}%` : `${videos.length > 0 ? 100 : 0}%` }} />
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="flex gap-10">
          {[
            ...
            <button
              ...
              className={`pb-6 text-[11px] font-black uppercase tracking-[0.2em] flex items-center gap-3 transition-all border-b-4 ${activeTab === tab.id ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'
                }`}
            >
          ...
          {collections.map(collection => (
            <div key={collection.id} className="group bg-white border border-slate-100 rounded-[56px] overflow-hidden hover:shadow-2xl transition-all cursor-pointer flex flex-col">
              ...
              <div className="p-12 space-y-8 flex-1 flex flex-col justify-between">
                ...
                <button
                  onClick={() => { /* Open collection detail view */ }}
                  className="w-full py-5 bg-slate-950 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.25em] hover:bg-indigo-600 transition-all shadow-xl shadow-slate-100 group-hover:-translate-y-1"
                >
      ...
      {(activeTab === 'All' || activeTab === 'Shared') && (
        <section className="space-y-10 relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
            {videos.length > 0 ? (
              videos.map(v => (
                <VideoCard key={v.id} video={v} onClick={onVideoClick} onDelete={onDeleteVideo} />
              ))
            ) : (
