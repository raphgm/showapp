              ...
              >
                {el.type === 'note' && (
                  <div className="flex-1 flex flex-col p-8 bg-white/40 backdrop-blur-sm rounded-[24px]">
                     ...
                     <textarea readOnly={isPresenting || el.locked} className="w-full flex-1 bg-transparent resize-none outline-none font-bold text-2xl leading-snug select-text scrollbar-hide text-slate-800 placeholder:text-slate-200" value={el.content} onChange={(e) => setElements(els => els.map(item => item.id === el.id ? { ...item, content: e.target.value } : item))} onPointerDown={e => { e.stopPropagation(); setSelection(new Set([el.id])); }} placeholder="INITIALIZING..." />
                  </div>
                )}
                {el.type === 'text' && <input readOnly={isPresenting || el.locked} className={`w-full h-full bg-transparent text-5xl font-black outline-none select-text text-center tracking-tighter ${el.color === 'transparent' ? 'text-indigo-600/80' : 'text-slate-900/80'}`} value={el.content} onChange={(e) => setElements(els => els.map(item => item.id === el.id ? { ...item, content: e.target.value } : item))} onPointerDown={e => { e.stopPropagation(); setSelection(new Set([el.id])); }} />}
